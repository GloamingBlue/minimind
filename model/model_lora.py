...
class LoRA(nn.Module):
    def __init__(self, in_features, out_features, rank):
        ...
        self.A = nn.Linear(in_features, rank, bias=False)  # 低秩矩阵A,实际shape(r, in)
        self.B = nn.Linear(rank, out_features, bias=False)  # 低秩矩阵B,实际shape(out, r)
        # 矩阵A高斯初始化
    ...
    def forward(self, x):
        return self.B(self.A(x))  # 因此这里使用B @ (A @ x),shape为(out, in),与原始的Linear一致
